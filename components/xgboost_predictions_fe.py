#taken from https://github.com/phunterlau/kaggle_higgs/blob/master/add_features.py
'''
...
def mas_combining1(row):
    rows = ['DER_mass_MMC','DER_mass_transverse_met_lep']
    frame = row[rows[0]] + row[rows[1]]
    mask = np.bitwise_or(row[rows[0]]==-999, row[rows[1]]==-999.)
    frame[mask] = -999.
    frame.name = 'MNew_mc1'
    return frame

def mas_combining2(row):
    rows = ['DER_mass_MMC','DER_mass_vis']
    frame = row[rows[0]] + row[rows[1]]
    mask = np.bitwise_or(row[rows[0]]==-999, row[rows[1]]==-999.)
    frame[mask] = -999.
    frame.name = 'MNew_mc2'
...
def mas_combining3(row):
    rows = ['DER_mass_transverse_met_lep','DER_mass_vis']
    frame = row[rows[0]] + row[rows[1]]
    mask = np.bitwise_or(row[rows[0]]==-999, row[rows[1]]==-999.)
    frame[mask] = -999.
    frame.name = 'MNew_mc3'
...
def add_features(df):
    ...
    new_features.extend([mas_combining1,mas_combining2,mas_combining3,mas_combining4])
